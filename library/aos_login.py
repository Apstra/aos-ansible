...
def aos_login(module):
    ...
    for attempt in range(MAX_ATTEMPTS):
        try:
            response = requests.post(aos_url, data=json.dumps(payload), headers=headers, verify=False)

            if response.status_code == 201:
                aos_session = {"server": mod_args['server'],
                               "token": response.json()['token']}
            elif response.status_code == 401:
                module.fail_json(msg="AOS-server login failed: Invalid Credentials")
            else:
                module.fail_json(
                    msg="AOS-server {} API not available/reachable, check server".format(aos_url))
